...
\item Are we offering Parquet files? --- No in DP0.1; likely yes in DP0.2. Currently our SDMified Parquet-generating pipelines are HSC only and Gen2 only. If Parquet files are offered the access will be via the read-only Butler Gen3 repo.

\item We should presumably explicitly rule out bulk download  --- YES.
We have ruled out offering bulk download facilities for DP0.1.  They are unlikely to be available at any time during the DP0 era, but we are still evaluating that against the time scale of the Data Previews and the schedule for the remaining required development effort. Making bulk download available had previously been discussed as a possible mitigation for the absence of user-facing batch compute in DP0. We still need to determine whether we need to provide some form of bulk access to DESC to return the Rubin-processed DC2 data from DP0.2 to them for their own use, both for scientific purposes and to allow them to develop their capability to ingest and work with bulk download.
